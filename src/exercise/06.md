...
- useDebugValue only works with custom made hooks and does not appear in production builds. Only implemented within the hook itself.
- Connects to show debug values through the React Developer Tools, specific to the custom hooks for any shown custom components using that hook.
- The useDebugValue also takes a second arg for formatting, only runs computation in devtools instead of taking up main thread. 
- React.useDebugValue(variables, ()=>formattedOutput)
